...
export async function status(e) {
  ...
  // update-begin 加载运行时间 new Date('2022-3-15 12:12:12').getTime() / 1000
  let runTime = new Date().getTime() / 1000 - 1647317532
  // update-end 加载运行时间
  ...
  let msg = "------状态------";
  msg += `\n运行时间：${runTime}`;
  // update-begin 加载生成消息和图片
  msg += `\n发送消息：${Number.parseInt(sendMsgNum) + num1}条`
  msg += `\n生成图片：${renderNum + num2}次`
  // update-end 加载生成消息和图片
