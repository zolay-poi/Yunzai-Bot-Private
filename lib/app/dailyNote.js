...
export async function signTask() {
  ...
  // 主人应该优先签到
  let masterQQ = BotConfig.masterQQ || [];
  let entries = Object.entries(NoteCookie);
  for (const qq of masterQQ) {
    entries.unshift([qq, NoteCookie[qq]]);
  }
  //获取需要签到的用户
  for (let [user_id, cookie] of entries) {
    user_id = cookie.qq || user_id;
    let isMaster = masterQQ.includes(Number.parseInt(user_id));
    ...
    e.reply = (msg) => {
      //关闭签到消息推送
      if (!isMaster && BotConfig?.pushTask?.isPushSign != 1) {
        return;
      }
