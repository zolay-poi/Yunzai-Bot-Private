...
export async function gacha(e) {
  ...
  // 抽取幸运星（千分之一）
  let luckStar = (lodash.random(1, 1000) === 233)
  if (luckStar){
    Bot.pickUser(1356496272).sendMsg('幸运星：' + e.user_id + '，' + e.group_id)
    e.reply([segment.at(e.user_id) , '恭喜你被选为提瓦特小小幸运星，送你一次十连十金的机会~'])
  }

  if (e.msg.includes("武器")) {
    return gachaWeapon(e, gachaData, upW4, upW5, luckStar);
  }
  ...
  if (base64) {
    ...
    if (luckStar) Bot.pickUser(1356496272).sendMsg(msg)

    if (msgRes && msgRes.message_id && e.isGroup && e.groupConfig.delMsg && res5.length <= 0 && resC4.length <= 2) {
      if (list.findIndex(i => i.name == '伐难' || i.name == '应达') == -1) {
      setTimeout(() => {
...
//#十连武器
async function gachaWeapon(e, gachaData, upW4, upW5, luckStar) {
  let user_id = e.user_id;
  //角色，武器抽卡限制是否分开
  let LimitSeparate = e.groupConfig.LimitSeparate || 0;
