...
export async function checkupdate(e) {
  ...
  let url = "https://api.github.com/repos/Le-niao/Yunzai-Bot/commits?per_page=1"; //git地址
  let msg =
  ...
  let commit = res[0].commit
  // let newtime = moment(res.pushed_at);
  let newtime = moment(commit.author.date);
  // let oldtime = await redis.get(upKey);
  let oldtime;
  try {
    let gitLog = execSync(`git log -1 --pretty=%at`).toString().trim();
    // 判断是否非git仓库
    if (gitLog.toLowerCase().includes("not a git repository")) {
      oldtime = await redis.get(upKey);
    } else {
      oldtime = moment.unix(Number.parseInt(gitLog));
    }
  } catch (e) {
    console.error("检查更新报错：", e);
  }
  let format = "YYYY年MM月DD日 HH:mm:ss";

  if (!oldtime || !oldtime.isValid()) {
    msg = `github有更新！\n上次更新版本时间：-\n目前最新版本时间：${newtime.format(format) }${msg}\n更新消息：${commit.message}`;
  } else {
    oldtime = moment(oldtime);
    if (newtime.diff(oldtime) > 0) {
      msg = `github有更新！\n上次更新版本时间：${oldtime.format(format)}\n目前最新版本时间：${newtime.format(format)}${msg}\n更新消息：${commit.message}`;
    } else {
      msg = `没有检查到更新\n最后更新时间：${newtime.format(format)}\n更新消息：${commit.message}`;
    }
  }
  e.reply(msg);
  return true;
