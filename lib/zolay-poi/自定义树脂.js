...
const resPath = path.join(_path, "resources/zolay-poi/dailyNote");

let role_user = Data.readJSON(resPath, "json/dispatch_time");
...
export async function dailyNote_zolay(e) {
  ...
  let realUid = uid === 508999631 ? 506030831 : uid;

  const response = await getDailyNote(realUid, cookie);
  if (!response.ok) {
    e.reply("米游社接口错误");
    return true;
  }
  ...
  if (salt <= 20) {
    // 有右下角表情（可莉）
    imgs = 'bg.png';
  } else if (salt <= 50 && salt > 20) {
    faceName = '志琼.png';
  } else {
    faceName = getRandomFace();
  }

  let base64 = await render("zolay-poi", `dailyNote`, {
    save_id: uid,
    uid: uid,
    coinTime_mb2Day,
    coinTime_mb2,
    ...
    resPath: `file:///${resPath.replace(/\\/g, "/")}`,
    faceName,
  }, "jpeg");
  if (base64) {
...
async function dateTime_(time) {
  ...
  return hh < 6 ? "凌晨" : hh < 12 ? "上午" : hh < 20 ? "下午" : "晚上";
}
...
const faceFiles = fs.readdirSync(path.join(resPath, "face"));

function getRandomFace() {
  return faceFiles[lodash.random(0, faceFiles.length - 1)];
}
