...
async function dealMsg(e) {
  ...
  if (e.groupConfig.disable && e.groupConfig.disable.length > 0) {
    if (lodash.intersection(e.groupConfig.disable, ['all']).length > 0) {
      return;
    }
  }
  ...
  for (let val of tmpCommand) {
    ...
    //原神查询功能时，@机器人代替#号
    if ((val.type === 'mysInfo' || val.hashMark) && e.atBot && e.msg) {
      e.msg = '#' + e.msg.replace(/^#/, '');
    }
    if (val.replyMark && e.hasReply && e.msg) {
      e.msg = '#' + e.msg.replace(/^#/, '');
    }
    ...
    let regexp = new RegExp(val.reg)
    if (regexp.test(e.msg) || val.reg === "noCheck" || (e.isPoke && val.reg === "#poke#")) {
      let log = `${e.logText} ${e.msg}:${val.name}`;
...
async function dealIncrease(e) {
  ...
  YunzaiApps.welcome.welcome(e).catch((err) => {
    Bot.logger.error(err);
  });
}
